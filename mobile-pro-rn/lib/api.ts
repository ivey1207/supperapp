...
export async function updateOrderStatus(token: string, orderId: string, status: string, finalAmount?: number, photoAfterUrl?: string) {
  const params: any = { status };
  if (finalAmount) params.finalAmount = finalAmount;
  if (photoAfterUrl) params.photoAfterUrl = photoAfterUrl;
